...
Это self-hosted приложение: домашний сервер, NAS, VPS или обычный ПК.
...
LibShelf не заменяет хранилище книг, а индексирует его. `.inpx` читается в собственный индекс; FB2 по запросу берётся из исходных zip/7z — так открываются карточка, обложка, читалка и скачивание. В папке данных лежат индекс, кэш обложек и, если включён вход, аккаунты, полки и прогресс. Архивы программа не переписывает. После импорта `.inpx` серверу уже не нужен: остаются папки с архивами и папка данных.
...
В поиске сверху показываются подходящие авторы; в расширенном — год издания и дата добавления. «ё/е» и порядок «Имя Фамилия» / «Фамилия Имя» не мешают, выдача постраничная. Авторы и серии — по буквам; в жанрах — «Популярные», «Новинки», «По алфавиту»; на странице автора — его серии. Читалка: страницы (свайп вбок или вниз) или лента, шрифт, выравнивание, полный экран, прогресс. На карточках при входе виден процент прочитанного. Если в FB2 нет обложки — заглушка с названием. Архивы могут лежать в нескольких папках. Языки каталога задаёт администратор (по умолчанию русский; можно несколько или все). Вход — с ролями читатель / админ или без пароля.
