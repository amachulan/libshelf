...
- поиск по **автору, названию и серии** сразу; расширенный — отдельно автор и **год издания**; **ё/е** и порядок имени не мешают; выдача по релевантности, с **пагинацией**
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/api/search?q=&title=&author=&year=` | поиск (`year` / `year_from`+`year_to`) |
